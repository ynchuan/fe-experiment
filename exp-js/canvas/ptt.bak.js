...
Portrait.prototype.fixLastPicSlide = function() {
    ...
    var lastSlideDom = t[t.length - 2].content;
    var startx;
    var starty;
    //手指接触屏幕
    ...
    lastSlideDom.addEventListener("touchend", function(e) {
        ...
        switch (direction) {
            ...
            case 2:
                //alert("向下！")
                legend.render.gotoPage(t.length - 3);
                break;
            case 3:
                //alert("向左！")
                break;
...
var isOuterBrowser = (window.location.hash === '#toshare' || /festival_share/i.test(window.location.href) || /adinote/i.test(window.location.href));
...
if (!isSharePlatform && isOuterBrowser) {
    setTimeout(function() {
